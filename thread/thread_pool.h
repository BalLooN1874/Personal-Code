...
*  Get the total of threads in this pool
*/
size_t ThreadPool::get_threads_total( )
{
	return _thread_count;
}
...
*  Get the number of jobs left in the queue.
*/
size_t	ThreadPool::get_jobs_remaining( )
{
	std::lock_guard<std::mutex> gurad(_queue_mutex);
	return _task_queue.size();
}
	...
	*  of jobs, look to use `ThreadPool::wait_all`.
	*/
void ThreadPool::join_all_thread( bool WaitForAll/* = true*/ )
{
	if (!_finished)
	{
		if (WaitForAll)
...
*/
template<class F, class... Args>
auto ThreadPool::add_normal_task( F&& f , Args&&... args )->std::future<typename std::result_of<F( Args... )>::type>
{
	using return_type = typename std::result_of<F(Args...)>::type;
	auto task = std::make_shared< std::packaged_task<return_type()> >(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
	std::future<return_type> res = task->get_future();
	stThreadPoolInfo stInfo;
	stInfo.pFuncPtr = std::move([task]()->void{ (*task)(); });
	stInfo.nPriority = EP_Normal;
	{
		std::lock_guard<std::mutex> guard(_queue_mutex);
		auto itor = std::upper_bound(_task_queue.begin(), _task_queue.end(), stInfo, [&](const stThreadPoolInfo& a, const stThreadPoolInfo& b)->bool{return a.nPriority > b.nPriority; });
		_task_queue.insert(itor, std::move(stInfo));
	}
	++_jobs_left;
	_job_available_var.notify_one();
...
*/
template<class F, class... Args>
auto ThreadPool::add_critical_task( F&& f , Args&&... args )->std::future<typename std::result_of<F( Args... )>::type>
{
	using return_type = typename std::result_of<F(Args...)>::type;
	auto task = std::make_shared< std::packaged_task<return_type()> >( std::bind( std::forward<F>( f ) , std::forward<Args>( args )... ) );
	std::future<return_type> res = task->get_future();
...
*  otherwise wait for a signal from the main thread.
*/
void ThreadPool::next_task( stThreadPoolInfo& stInfo )
{
	std::unique_lock<std::mutex> job_lock( _queue_mutex );
	_job_available_var.wait( job_lock , [ this ] ()->bool
	{
